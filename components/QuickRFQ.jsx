  ...
  if (sent) {
    return (
      <div className="card" style={{ padding: 28, textAlign: "left" }}>
        ...
        <p style={{ marginTop: 8 }}>
          Reference <span className="mono" style={{ color: "var(--ink-700)" }}>{ref}</span> with <span className="mono" style={{ color: "var(--ink-700)" }}>{pns.length}</span> part {pns.length === 1 ? "number" : "numbers"} routed to <span className="mono">rfq@srmglobaltech.com</span>. Our team responds within 2 hours during business hours.
        </p>
        <button className="btn btn-ghost btn-sm" style={{ marginTop: 16 }} onClick={() => { setSent(false); setData({ name: "", company: "", email: "", qty: "", notes: "" }); setPns([]); }}>
          Submit another →
        </button>
      </div>
    );
